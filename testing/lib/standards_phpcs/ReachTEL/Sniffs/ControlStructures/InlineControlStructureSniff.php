 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class ReachTEL_Sniffs_ControlStructures_InlineControlStructureSniff implements PHP_CodeSniffer_Sniff
{
        ...
        if ($fix === true) {
            ...
            if ($next !== $end) {
                if ($endLine !== $end) {
					...
					// START Kevin Ohayon for ReachTEL fix
                    $phpcsFile->fixer->addContent($end, "\n" . str_repeat("\t", $tokens[$end]['level']) . '}');
                    // END Kevin Ohayon for ReachTEL fix
                }
            } else {
                if ($endLine !== $end) {
