<?php
/**
 * @author phillip.berry@equifax.com
 * @copyright ReachTel (ABN 40 133 677 933)
 */

namespace Services\Email\Dkim;

use Models\Email\Dkim\DkimKey;
use Services\Exceptions\Email\DkimException;
use testing\AbstractPhpunitTest;

/**
 * Class DkimKeyFactoryTest
 */
class DkimKeyFactoryTest extends AbstractPhpunitTest {

	private $testPrivate = "-----BEGIN RSA PRIVATE KEY-----

K5rXb8IrAUJbaTV00yzKwdR0AoPfkq48Pi4nePjLH6l1u6XekWdM
-----END RSA PRIVATE KEY-----";

	private $testPublic = "-----BEGIN PUBLIC KEY-----

TwIDAQAB
-----END PUBLIC KEY-----";

	/**
	 * @return void
	 */
	public function testCreateKey() {
		$this->assertInstanceOf(
			DkimKey::class,
			(new DkimKeyFactory())->createKey($this->testPrivate)
		);

		$this->expectException(DkimException::class);
		(new DkimKeyFactory())->createKey($this->testPublic);
		(new DkimKeyFactory())->createKey("123");
	}

	/**
	 * @return void
	 */
	public function testCreateKeyFromResource() {
		$this->assertInstanceOf(
			DkimKey::class,
			(new DkimKeyFactory())->createKeyFromResource(openssl_pkey_get_private($this->testPrivate))
		);
		$this->expectException(DkimException::class);
		(new DkimKeyFactory())->createKeyFromResource(openssl_pkey_get_private($this->testPublic));
	}
}
