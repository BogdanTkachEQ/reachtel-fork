...
if (!empty($argv[1])) {
	...
	// We want to find a file with the file name format of MBFS-SMS-REACHTEL-DDDMMYYYY-HHMMSS.CSV
	// @TODO: We also match the old file name (without '-SMS'). Remove it once MBFS changed it on their side.
	$files = api_misc_sftp_list($options);
	if (!empty($files)) {
		$filenames = preg_grep(
			'/MBFS\-(SMS\-)?REACHTEL\-' . $date->format('dmY') . '/i',
			$files
		);
	}
}
...
if (!$filenames) {
	...
	$email['subject'] =
		'[ReachTEL] Auto-load error - MBFS - MBFS-SMS-REACHTEL-' . $date->format('dmY') . "-*.CSV.PGP";
	$email['content'] =
	...
	print 'File not found MBFS-SMS-REACHTEL-' . $date->format('dmY') . "-*.CSV.PGP\n";
	exit();
}
...
foreach($filenames as $filename) {
	...
	if (!api_misc_sftp_get($options)) {
		...
		$email['cc'] = 'ReachTEL Support <support@ReachTEL.com.au>';
		$email['from'] = 'ReachTEL Support <support@ReachTEL.com.au>';
		$email['subject'] = '[ReachTEL] Auto-load error - MBFS - MBFS-SMS-REACHTEL-' . $date->format('dmY') . '-*.CSV.PGP';
...
$campaign = array("name" => "MBFS-SMS-" . $date->format("jFy") . "-SFTP");
...
if(is_numeric($exists)) {
	...
	$previouscampaigns = api_campaigns_list_all(true, null, null, array("search" => "MBFS-SMS-*-SFTP"));
