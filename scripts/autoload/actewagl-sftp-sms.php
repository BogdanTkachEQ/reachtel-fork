...
if ($missingTags) {
	...
	$email["subject"] = "[ReachTEL] Auto-load error - ActewAGL - " . $filename;
...
while ($holidayCheckBack >= 0) {
	...
	$filename = "SMSREQUEST_ACTEWAGL_" . $checkDate->format('Ymd') . ".csv";
	$path = "/tmp/";
...
if (count($goodDates) > 0) {
	...
	$campaignname = "ActewAGL-SMS-" . $date->format("Ymd");
	print $campaignname;
	...
	$previouscampaigns = api_campaigns_list_all(true, null, null, array("search" => "ActewAGL-SMS-20"));
