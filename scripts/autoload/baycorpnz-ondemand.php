...
if(api_misc_ispublicholiday("AKL")){
    print "Stopping because it is a AKL public holiday\n";
    exit;
}
...
if(!api_misc_sftp_get($options)){
	...
	$email["subject"] = "[ReachTEL] Auto-load error - BaycorpNZ - " . $filename;
...
if(preg_match("/^SMS_Request_(\d{2})(\d{2})(\d{4})_(\d{2})(\d{2})(\d{2})\.csv$/i", $filename, $matches)) {
	...
	$campaignname = "BaycorpNZ-" . date("jFy", $time) . "-OnDemand-" . $matches[4] . $matches[5] . $matches[6];
	$search = "BaycorpNZ-*-OnDemand";
