...
if (!empty($argv[1])) {
	...
	// We want to find a file with the file name format of DTFS-PHONE-REACHTEL-DDDMMYYYY-HHMMSS.CSV
	$files = api_misc_sftp_list($options);
	if (!empty($files)) {
		$filenames = preg_grep(
			'/DTFS\-PHONE\-REACHTEL\-' . $date->format('dmY') . '/i',
			$files
		);
	}
}
...
if (!$filenames) {
	...
	$email['subject'] =
		'[ReachTEL] Auto-load error - DTFS - DTFS-PHONE-REACHTEL-' . $date->format('dmY') . '-*.CSV.PGP';
	$email['content'] =
	...
	print 'File not found DTFS-PHONE-REACHTEL-' . $date->format('dmY') . "-*.CSV.PGP\n";
	exit();
}
...
foreach($filenames as $filename) {
	...
	if (!api_misc_sftp_get($options)) {
		...
		$email['cc'] = 'ReachTEL Support <support@ReachTEL.com.au>';
		$email['from'] = 'ReachTEL Support <support@ReachTEL.com.au>';
		$email['subject'] = '[ReachTEL] Auto-load error - DTFS - DTFS-PHONE-REACHTEL-' . $date->format('dmY') . '-*.CSV.PGP';
...
$campaign_base = 'DTFS-IVR-' . $date->format('jFy');
