...
$tags = api_cron_tags_get(CRON_ID_ENV_KEY) ?: [];
...
if (isset($tags['run-date'])) {
    try {
        $startdate = new DateTime($tags['run-date'], new DateTimeZone($timezone));
    }catch(\Exception $e){
        print "Invalid run date given: '".$tags['run-date']."' check https://www.php.net/manual/en/datetime.formats.php for valid formats.\n";
        exit;
    }
}else{
    $startdate = new DateTime('First day of last month Midnight', new DateTimeZone($timezone));
}

if (isset($tags['end-run-date'])) {
    try {
        $enddate = new DateTime($tags['end-run-date'], new DateTimeZone($timezone));
    }catch(\Exception $e){
