<?php
/**
 * dtfs cascading post complete hook
 *
 ...
 * @return void
 */
function api_campaigns_hooks_dtfs_cascade_campaign($campaign_id) {
	$log_array = [
		'request_id' => uniqid(),
		'campaign_id' => $campaign_id,
	];
	$start_time = $checkpoint_time = _dtfs_cascade_campaign_timer_start();
	$checkpoint_time = _dtfs_cascade_campaign_log('Start post hook', $log_array, $start_time, $checkpoint_time);

	$current_campaign_name = api_campaigns_setting_getsingle($campaign_id, 'name');
	$current_campaign_number = _dtfs_cascade_campaign_get_campaign_number($current_campaign_name);

	if ($current_campaign_number === 3) {
		return;
	}

	$next_campaign_name = _dtfs_cascade_campaign_get_next_campaign_name($current_campaign_name);
	$next_campaign_number = _dtfs_cascade_campaign_get_campaign_number($next_campaign_name);
	...
	if ($next_campaign_name !== false) {
		...
		$checkpoint_time = _dtfs_cascade_campaign_log('Adding targets', $log_array, $start_time, $checkpoint_time);
		foreach ($targets as $target_id => $destination) {
			...
			$next_destination = _dtfs_cascade_campaign_get_destination($next_campaign_number, $merge_data);
			$next_target_key = _dtfs_cascade_campaign_get_next_target_key($current_campaign_number, $target['targetkey']);
			...
			$well_known_fields = _dtfs_cascade_campaign_get_well_known_fields($next_campaign_number, $merge_data);
		...
		$checkpoint_time = _dtfs_cascade_campaign_log('Added targets', $log_array, $start_time, $checkpoint_time);
		...
		_dtfs_cascade_campaign_log('End post hook', $log_array, $start_time, $checkpoint_time);
 ...
 * @return float
 */
function _dtfs_cascade_campaign_timer_start() {
 ...
 * @return string
 */
function _dtfs_cascade_campaign_timer_elapsed($start_time) {
 ...
 * @return float
 */
function _dtfs_cascade_campaign_log($message, array $log_array, $start_time, $checkpoint_time) {
	$log_array['time_elapsed'] = _dtfs_cascade_campaign_timer_elapsed($checkpoint_time);
	$log_array['total_time_elapsed'] = _dtfs_cascade_campaign_timer_elapsed($start_time);
	api_misc_audit('DTFS_CASCADE_CAMPAIGN', sprintf("%s:\t%s", $message, json_encode($log_array)));
	return _dtfs_cascade_campaign_timer_start();
}
 ...
 */
function _dtfs_cascade_campaign_get_next_campaign_name($campaign_name) {
	$suffix_1 = 'SFTP';
	...
	if (preg_match('/^DTFS-IVR-.*-\w{4,8}$/', $campaign_name) !== 1) {
		return api_error_raise($campaign_name . " doesn't match expected format for dtfs cascade");
	}
 ...
 * @return string|boolean
 */
function _dtfs_cascade_campaign_get_next_target_key($campaign_number, $target_key) {
 ...
 * @return integer
 */
function _dtfs_cascade_campaign_get_campaign_number($campaign_name) {
	$suffix_1 = 'SFTP';
	$suffix_1_length = strlen($suffix_1);
	$suffix_2 = 'Contact2';
	$suffix_2_length = strlen($suffix_2);
 ...
 * @return string|boolean
 */
function _dtfs_cascade_campaign_get_destination($campaign_number, array $merge_data) {
	$destination_field = '';
	switch ($campaign_number) {
		case 1:
 ...
 */
function _dtfs_cascade_campaign_get_well_known_fields($campaign_number, array $merge_data) {
	$data = [];
	switch ($campaign_number) {
		case 1:
			if (!(
