...
function api_sms_send_supplier_23($from, $to, $content, $eventid = null, $options = array()) {
    global $statsd;
    ...
    if ($result === false) {
        $statsd->increment('morpheus.sms.send.sinchsmpp.errors.smpp');
        return api_error_raise('SMS send failed - Sinch SMPP');
    } else {
        $result = unserialize($result);
    }

    $statsd->timing('morpheus.sms.send.sinchsmpp', (microtime(true) - $starttime) * 1000);
