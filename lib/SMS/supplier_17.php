...
function api_sms_send_supplier_17($from, $to, $content, $eventid = null, $options = array()){
	...
	if($result === false) {
		...
		$statsd->increment("morpheus.sms.send.clx.errors.smpp");
		return api_error_raise("SMS send failed - CLX SMPP");

	} else $result = unserialize($result);

	$statsd->timing("morpheus.sms.send.clx", (microtime(true) - $starttime)*1000);
