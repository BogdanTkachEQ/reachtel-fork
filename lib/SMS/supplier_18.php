...
function api_sms_send_supplier_18($from, $to, $content, $eventid = null, $options = array()){
	...
	if($result === false) {
		...
		$statsd->increment("morpheus.sms.send.pivotel.errors.smpp");
		return api_error_raise("SMS send failed - Pivotel SMPP");

	} else $result = unserialize($result);

	$statsd->timing("morpheus.sms.send.pivotel", (microtime(true) - $starttime)*1000);
