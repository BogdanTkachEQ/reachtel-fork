...
class Xero {
	...
	public function __call($name, $arguments) {
		...
		$valid_methods = array('attachments','accounts','contacts','creditnotes','currencies','invoices','organisation','payments','taxrates','trackingcategories','items','banktransactions','brandingthemes');
		$valid_post_methods = array('banktransactions','contacts','creditnotes','expenseclaims','invoices','items','manualjournals','receipts');
		$valid_put_methods =  array('attachments','payments');
		$valid_get_methods =  array('attachments','accounts','banktransactions','brandingthemes','contacts','creditnotes','currencies','employees','expenseclaims','invoices','items','journals','manualjournals','organisation','payments','receipts','taxrates','trackingcategories','users');
		$methods_map = array(
		...
		if ( (count($arguments) == 0) || ( is_string($arguments[0]) ) || ( is_numeric($arguments[0]) ) || ( $arguments[0] === false ) ) {
			...
			if(!empty($arguements[1]) AND ($arguments[1]!=false)) $modified_after = ( count($arguments) > 1 ) ? str_replace( 'X','T', date( 'Y-m-dXH:i:s', strtotime($arguments[1])) ) : false;
			if(!empty($arguements[2]) AND ($arguments[2]!=false)) $where = ( count($arguments) > 2 ) ? $arguments[2] : false;
			...
			$method = $methods_map[$name];
			if ($name == "attachments") {
				$filename = str_replace(array("(", ")", "&", "!", "'"), array("[", "]", " and ", "", ""), $arguments[0]["filename"]);
				$method = $arguments[0]["endpoint"] . "/" . $arguments[0]["guid"] . "/" . $method . "/" . rawurlencode($filename);
				$post_body = $arguments[0]["content"];
			} elseif ( is_a( $arguments[0], 'SimpleXMLElement' ) ) {
				$post_body = $arguments[0]->asXML();
			} elseif ( is_array( $arguments[0] ) ) {
				$post_body = ArrayToXML::toXML( $arguments[0], $rootNodeName = $method );
			} else $post_body = null;

			if($name != "attachments") $post_body = trim(substr($post_body, (stripos($post_body, ">")+1) ));
