...
class BounceHandler
{
	...
	public $returnHeadersArray = false;
	...
	public function getResults()
	{
		...
		foreach ($searchResults as $messageId) {
			...
			if (preg_match ("/content-type.+multipart\/report.+delivery-status/is", $header, $matches)) {
...
// this is useful for reading back custom headers sent in the original email.
			$result['emailHeadersArray'] = array();
			if ($this->returnHeadersArray) {
				$result['emailHeadersArray'] = $this->getHeadersArray($header);
			}
...
// this is useful for reading back custom headers sent in the original email.
	...
	public function move($messageId, $mailbox)
	{
		return imap_mail_move($this->_connection, $messageId, $mailbox);
	}

	public function delete($messageId)
	{
		return imap_delete($this->_connection, $messageId);
	}
...
// Could be multi-line , if the new line is beginning with SPACE or HTAB
		if (preg_match ("/Diagnostic-Code:((?:[^\n]|\n[\t ])+)(?:\n[^\t ]|$)/is", $dsnReport, $matches)) {
			$diagnosticCode = preg_replace(array("/\r\n/", "/    /"), array("", ""), trim($matches[1]));
		}
