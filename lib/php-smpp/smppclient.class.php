...
class SmppClient
{
	...
	protected function parseSMS(SmppPdu $pdu)
	{
		...
		if(($esmClass & SMPP::ESM_UDHI) == SMPP::ESM_UDHI) { // Message has a UDHI
		...
		if($dataCoding == 0) {
			$message = GsmEncoder::gsm0338_to_utf8($message);
		...
		if (($esmClass & SMPP::ESM_DELIVER_SMSC_RECEIPT) != 0) {
			...
			$sms = new SmppSms($pdu->id, $pdu->status, $pdu->sequence, $pdu->body, $service_type, $source, $destination, $esmClass, $protocolId, $priorityFlag, $registeredDelivery, $dataCoding, $message, $tags, null, null, null, null, $udhi);
		}
	...
	protected function readPDU_resp($seq_number, $command_id)
	{
		...
		do{
			...
			if ($pdu) {
				...
				if ($pdu->id==SMPP::ENQUIRE_LINK) $this->sendPDU(new SmppPdu(SMPP::ENQUIRE_LINK_RESP, SMPP::ESME_ROK, $pdu->sequence, "\x00"));
				else array_push($this->pdu_queue, $pdu); // unknown PDU push to queue
			}
		} while($pdu);
		return false;
	}
...
class SmppDeliveryReceipt extends SmppSms
{
	...
	{
		$numMatches = preg_match('/^id:([^ ]+) sub:(\d{1,3}) dlvrd:(\d{1,3}) submit date:(\d{10,12}) done date:(\d{10,12}) stat:([A-Z ]{7}) err:(\w{1,3}) text:(.*)$/si', $this->message, $matches);
		if ($numMatches == 0) {
